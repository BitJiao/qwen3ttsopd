...
: "${INPUT_JSONL:?set INPUT_JSONL to OPD JSONL with target audio_codes or codes_path}"

COMMAND=("${PYTHON:-${ROOT}/.venv/bin/python}" -m qwen3opsd.qualify_teachers \
...
if [[ -n "${VD_TEACHER_MODEL_PATH:-}" ]]; then
  COMMAND+=(--vd_teacher_model_path "${VD_TEACHER_MODEL_PATH}")
fi
